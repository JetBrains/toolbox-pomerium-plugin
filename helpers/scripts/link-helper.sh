...
generate_link() {
  local pomerium_route="$1"
  local pomerium_port="$2"
  local pomerium_instance="$3"
  local agent_connection_url="$4"
  local connection_key_raw="$5"
  local agent_auth="$6"

  local pomerium_route_encoded agent_connection_url_encoded connection_key_encoded
  pomerium_route_encoded="$(urlencode "$pomerium_route")"
  agent_connection_url_encoded="$(urlencode "$agent_connection_url")"
  connection_key_encoded="$(urlencode "$connection_key_raw")"

  local link
  link="jetbrains://remote-dev/jetbrains.toolbox.pomerium/new-environment#pomeriumRoute=${pomerium_route_encoded}&connectionKey=${connection_key_encoded}&pomeriumPort=${pomerium_port}"
...
main() {
  ...
  while IFS= read -r defaults_line; do
    case "$defaults_line" in
      POMERIUM_ROUTE=*) POMERIUM_ROUTE="${defaults_line#POMERIUM_ROUTE=}" ;;
  ...
  local pomerium_route_default="${POMERIUM_ROUTE:-tcp+https://agent.localhost:443}"
  ...
  printf '  route: pomeriumRoute, pomeriumPort, pomeriumInstance (optional)\n'
  printf '  backend: connectionKey\n'
  printf '  agent: agentConnectionUrl, agentAuth\n'
  printf '  runtime: agent tcp port, backend relay port (optional)\n'
  printf '\n'

  local pomerium_route pomerium_port pomerium_instance agent_connection_url connection_key_raw agent_auth agent_tcp_listen_on_port backend_forward_port
  pomerium_route="$(prompt_default "pomeriumRoute" "$pomerium_route_default")"
  ...
  local link
  link="$(generate_link "$pomerium_route" "$pomerium_port" "$pomerium_instance" "$agent_connection_url" "$connection_key_raw" "$agent_auth")"

  printf '\n'
  log "Summary"
  printf '  defaults file: %s\n' "$DEFAULTS_FILE"
  printf '  pomeriumRoute: %s\n' "$pomerium_route"
  if [[ -n "$pomerium_instance" ]]; then
...
POMERIUM_ROUTE=$(python3 -c 'import shlex,sys; print(shlex.quote(sys.argv[1]))' "$pomerium_route")
