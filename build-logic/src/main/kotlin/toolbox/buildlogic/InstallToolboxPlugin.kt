...
class InstallToolboxPlugin : Plugin<Project> {
    override fun apply(target: Project) {
        ...
        installTask.configure { dependsOn(target.tasks.named("shadowJar")) }
    }
    ...
    abstract class InstallTask : DefaultTask() {
        ...
        fun install() {
            ...
            project.sync {
                ...
                val toolboxCachesDir = when {
                    os.contains("win") -> System.getenv("LOCALAPPDATA")?.let { Path.of(it) }
                        ?: (userHome / "AppData" / "Local")

                    os.contains("linux") -> System.getenv("XDG_DATA_HOME")?.let { Path.of(it) }
                ...
                val targetDir = pluginsDir / resolveExtensionId()

                // Copy jar task output and the generated JSON
                from(project.tasks.getByName("shadowJar"))
        ...
        private fun resolveExtensionId(): String {
            val fallback = extensionId.get()
            val json = extensionJsonFile.get().asFile.readText()
            val match = Regex(""""id"\s*:\s*"([^"]+)"""").find(json)
            return match?.groupValues?.get(1)?.takeIf { it.isNotBlank() } ?: fallback
        }
    }
}
